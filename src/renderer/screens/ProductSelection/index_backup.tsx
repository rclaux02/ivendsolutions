...
const filterCategoriesData = [
  {
    name: 'Categoria',
    options: [
      { id: '1x ECO Nano Pod Kit\n1x ECO Nano Pod Kit (0.8ohm)\n1x Manual de usuario', label: 'ECO Nano Pod Kit (0.8ohm)' },
      { id: '1x ECO Nano Pod Kit-2\n1x ECO Nano Pod Kit (0.4ohm)\n1x Manual de usuario', label: 'ECO Nano Pod Kit (0.4ohm)' },
  ...
  },
  {
    name: 'Marca',
    options: [
      { id: 'Coca Cola Company', label: 'Coca Cola Company' },
      { id: 'Frito Lay', label: 'Frito Lay' },
      { id: 'Mars', label: 'Mars' },
      { id: 'Nestle', label: 'Nestle' },
      { id: 'Otros', label: 'Otros' },
      { id: 'Picaras', label: 'Picaras' },
      { id: 'Prezlet', label: 'Prezlet' },
      { id: 'Sayon', label: 'Sayon' },
    ],
  },
  {
    name: 'Precio',
...
const ProductSelection: React.FC = () => {
  ...
  useEffect(() => {
    if (dbProducts && dbProducts.length > 0) {
      ...
      setFilterCategories(prevCategories => {
        ...
        // Find the index of the "Marca" category
        const brandCategoryIndex = updatedCategories.findIndex(category => category.name === 'Marca');
      ...
      // Clean up invalid filters and apply valid ones
      const cleanedFilters = cleanInvalidFilters(activeFilters, filterCategories);
      if (JSON.stringify(cleanedFilters) !== JSON.stringify(activeFilters)) {
        setActiveFilters(cleanedFilters);
        applyFiltersToProducts(dbProducts, cleanedFilters);
      } else {
  ...
  const handlePaymentComplete = async () => {
    try {
      ...
      // Cerrar modal de pago - NO abrir SuccessModal todavía
      ...
      for (const item of cartItems) {
        ...
        // ✅ OPTIMIZADO: No actualizar estado durante el procesamiento para evitar re-renders
        // Solo se actualiza al final del proceso completo
        // setDispensingState({
        //   totalProducts: totalProductUnits,
        ...
        // ✅ ABRIR SUCCESSMODAL DESPUÉS DE CONFIRMAR QUE HAY SLOTS DISPONIBLES
        if (dbResult.success && !showSuccessModal) {
          console.log('[ProductModal] 🚀 Abriendo SuccessModal después de confirmar slots disponibles');
          
          // Delay de 0.5 segundos antes de abrir el modal
          setTimeout(() => {
            setShowSuccessModal(true);
            console.log('[ProductModal] 🚀 SuccessModal abierto después de 0.5s de delay');
          }, 500);
        }
        ...
        if (isMultiSlot && usedSlots) {
          ...
          for (const slotInfo of usedSlots) {
            ...
            for (let i = 0; i < slotQuantity; i++) {
              ...
              setDispensingState({
                totalProducts: totalProductUnits,
                currentProductIndex: unitsDispensedSoFar + successfulDispenses + 1,
                currentProductName: item.product.FS_SABOR || item.product.name
              });
          ...
          for (let i = 0; i < quantityDispensed; i++) {
            ...
            // ✅ OPTIMIZADO: No actualizar estado en cada unidad para evitar re-renders
            // Solo se actualiza al final del proceso completo
            // setDispensingState({
            //   totalProducts: totalProductUnits,
            //   currentProductIndex: unitsDispensedSoFar + successfulDispenses + 1,
        ...
        // Add delay between different products to prevent hardware conflicts
        if (cartItems.indexOf(item) < cartItems.length - 1) {
          console.log('Waiting 500 milliseconds before processing next product...');
      ...
      // CRÍTICO: Forzar que el SuccessModal cambie a estado 'dispensed' para mostrar las caritas
      // Esto asegura que el modal no se quede en estado 'dispensing' indefinidamente
      setDispensingState({
        totalProducts: totalProductUnits,
        currentProductIndex: totalProductUnits, // Forzar que sea igual al total
      ...
      console.log('[ProductSelection] ✅ Dispensing completed successfully. SuccessModal will now show feedback options.');
      console.log('[ProductSelection] ✅ DispensingState updated:', {
        totalProducts: totalProductUnits,
  ...
  useEffect(() => {
    console.log('useEffect de temperatura montado');
    if (window.electron && window.electron.ipcRenderer) {
      console.log('Solicitando temperatura...');
      window.electron.ipcRenderer.invoke('temp:getCurrent').then((temp) => {
        console.log('Temperatura recibida:', temp);
        setTemperature(temp !== null ? String(temp) : null);
      });
    } else {
      console.log('window.electron o ipcRenderer no está disponible');
    }
  }, []);
  ...
  const testFeedbackWithRealTransactionId = async () => {
    console.log('[ProductSelection] Testing feedback with real transaction ID: 0000522302205470');
    
    try {
      // Test happy feedback
      const happyResult = await window.electron.ipcRenderer.invoke('purchase:submitFeedback', {
        paymentTransactionId: '0000522302205470',
        feedbackValue: 'happy',
        feedbackReason: null
      });
      ...
      const sadResult = await window.electron.ipcRenderer.invoke('purchase:submitFeedback', {
        paymentTransactionId: '0000522302205470',
        feedbackValue: 'sad',
        feedbackReason: 'Producto sin refrigeración'
      });
              ...
              <div className={`grid ${selectedProduct ? 'grid-cols-3' : 'grid-cols-5'} gap-4`}>
                {filteredProducts.map((product, index) => (
                  ...
                  >
                    ...
                    {product.discount && !isNaN(Number(product.discount)) && Number(product.discount) > 0 && (
